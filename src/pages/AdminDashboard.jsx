...
const API_BASE = '';
const inputClass = 'w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500';
...
export default function AdminDashboard() {
    ...
    if (loading) {
        return <div className="min-h-screen bg-slate-50"><Navbar /><div className="absolute inset-0 flex items-center justify-center"><div className="w-16 h-16 border-4 border-slate-200 border-t-blue-600 rounded-full animate-spin" /></div></div>;
    }

    return (
        <div className="min-h-screen bg-slate-50 relative overflow-hidden font-sans">
            <Navbar />
            <div className="absolute top-[-10%] right-[-5%] w-[40%] h-[400px] bg-blue-200/35 rounded-full blur-[100px] pointer-events-none" />
            <div className="absolute bottom-[-10%] left-[-5%] w-[50%] h-[500px] bg-sky-100/45 rounded-full blur-[120px] pointer-events-none" />
            ...
            <div className="pt-28 pb-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-10">
                    ...
                    <button onClick={() => openModal()} className="px-6 py-3 rounded-2xl bg-gradient-to-r from-blue-600 to-blue-700 text-white font-black shadow-md shadow-blue-500/25 hover:shadow-blue-500/35 transition-shadow">Create Event</button>
                </div>
                    ...
                    <Panel title="Event Performance" subtitle="Event dengan revenue tertinggi">
                        <div className="space-y-3">
                            {(analytics?.sales_by_event || []).map((item) => (
                                <div key={item.ticket_id} className="rounded-2xl border border-slate-200 bg-white p-4 flex justify-between gap-4">
                    ...
                    <Panel title="Payment Methods" subtitle="Aktif/nonaktifkan gateway tertentu">
                        <div className="space-y-3">
                            {paymentMethods.map((method) => (
                                <div key={method.id} className="rounded-2xl border border-slate-200 bg-white p-4 flex justify-between gap-4">
                                    <div><p className="font-black text-slate-900">{method.name}</p><p className="text-sm text-slate-500">{method.description}</p></div>
                                    <button onClick={() => handleTogglePaymentMethod(method)} className={`px-4 py-2 rounded-full text-xs font-black border ${method.is_active ? 'bg-emerald-500/30 text-emerald-400 border-emerald-500/60' : 'bg-slate-100 text-slate-500 border-slate-300'}`}>{method.is_active ? 'Aktif' : 'Nonaktif'}</button>
                                </div>
                            ))}
                        </div>
                    </Panel>
                    ...
                    <Panel title="Manage Events" subtitle="Cari, edit, dan kelola status event">
                        ...
                        <div className="space-y-4">
                            {filteredTickets.map((ticket) => (
                                <div key={ticket.id} className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm">
                                    <div className="flex flex-col lg:flex-row justify-between gap-4">
                                        <div className="flex gap-4 min-w-0">
                                            ...
                                            <div className="min-w-0">
                                                ...
                                                <p className="text-sm text-slate-500">{ticket.city || 'TBA'} â€¢ {ticket.venue || 'Venue TBA'}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-3 flex-wrap">
                                            ...
                                            <button onClick={() => openModal(ticket)} className="px-4 py-2 rounded-xl bg-blue-100 text-blue-700 font-black text-sm border border-blue-300">Edit</button>
                    ...
                    <div className="space-y-8">
                        <Panel title="Voucher Builder" subtitle="Tambah promo untuk mendorong conversion">
                            <form onSubmit={handleCreateVoucher} className="space-y-4">
                                ...
                                <label className="inline-flex items-center gap-3 text-sm font-bold text-slate-900"><input type="checkbox" checked={voucherData.is_active} onChange={(e) => setVoucherData((prev) => ({ ...prev, is_active: e.target.checked }))} />Active sekarang</label>
                                <button type="submit" disabled={voucherSaving} className="w-full rounded-2xl bg-blue-600 text-white py-3 font-black">{voucherSaving ? 'Saving...' : 'Simpan Voucher'}</button>
                            </form>
                        </Panel>
                        <Panel title="Voucher Library" subtitle="Promo yang sudah dibuat">
                            <div className="space-y-3">
                                {vouchers.map((voucher) => (
                                    <div key={voucher.id} className="rounded-2xl border border-slate-200 bg-white p-4">
                                        ...
                                        <p className="mt-3 text-sm text-slate-500">Used: <span className="font-black">{voucher.used_count}</span></p>
                                    </div>
                                ))}
                            </div>
                        </Panel>
            ...
            {showModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    <div className="fixed inset-0 bg-slate-900/25 backdrop-blur-sm" onClick={() => setShowModal(false)} />
                    <motion.div initial={{ opacity: 0, scale: 0.96, y: 12 }} animate={{ opacity: 1, scale: 1, y: 0 }} className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-[2rem] bg-white border border-slate-200 p-8 shadow-2xl">
                        <form onSubmit={handleSaveTicket} className="space-y-4">
                            <h2 className="text-3xl font-display font-black text-slate-900">{editMode ? 'Edit Event' : 'Create Event'}</h2>
                            <div onClick={() => fileInputRef.current?.click()} className="rounded-[1.75rem] border-2 border-dashed border-slate-300 bg-slate-50 cursor-pointer overflow-hidden">{imagePreview ? <img src={imagePreview} alt="Preview" className="w-full h-52 object-cover" /> : <div className="h-52 flex items-center justify-center text-slate-500 font-bold">Klik untuk upload gambar</div>}</div>
                            ...
                            <div className="flex flex-col-reverse sm:flex-row gap-3">
                                <button type="button" onClick={() => setShowModal(false)} className="rounded-2xl border border-slate-300 px-5 py-3 font-black text-slate-700">Cancel</button>
                                <button type="submit" disabled={saving} className="rounded-2xl bg-gradient-to-r from-blue-600 to-blue-700 text-white px-6 py-3 font-black">{saving ? 'Saving...' : editMode ? 'Save Changes' : 'Publish Event'}</button>
                            </div>
                        </form>
                    </motion.div>
                </div>
...
function Stat({ label, value }) {
    return <div className="bg-white border border-slate-200 rounded-[2rem] p-6 shadow-sm"><p className="text-xs font-black uppercase tracking-[0.22em] text-slate-500">{label}</p><p className="mt-3 text-3xl font-display font-black text-slate-900">{value}</p></div>;
}

function Panel({ title, subtitle, children }) {
    return <div className="bg-white border border-slate-200 rounded-[2.5rem] p-6 sm:p-8 shadow-sm"><h2 className="text-2xl font-display font-black text-slate-900">{title}</h2><p className="text-slate-500 text-sm mt-2 mb-6">{subtitle}</p>{children}</div>;
}

function Badge({ children, tone = 'primary' }) {
    const tones = { primary: 'bg-blue-100 text-blue-700 border-blue-300', emerald: 'bg-emerald-100 text-emerald-700 border-emerald-200', rose: 'bg-red-100 text-red-700 border-red-200', amber: 'bg-amber-100 text-amber-700 border-amber-200', slate: 'bg-slate-100 text-slate-500 border-slate-300' };
